...
# --- Mode Selection ---
while True:
    mode = input("Select mode (type 'conversation' for Normal Conversation or 'verse' for Verse Comparison, 'quit' to exit): ").strip().lower()
    ...
    if mode == 'quit':
        ...
        print("\nEntering Normal Conversation mode. Type 'exit' to go back to mode selection.\n")
        while True:
            user_prompt = input("You: ").strip()
            if user_prompt.lower() == 'exit':
                print("Exiting Normal Conversation mode...")
                break
            
            # Generate response
            inputs = tokenizer(user_prompt, return_tensors="pt", padding=True, truncation=True, max_length=512)
        ...
        # Verse Comparison Mode (existing functionality)
        print("\nEntering Verse Comparison mode. Type 'exit' to go back to mode selection.\n")
        while True:
            user_input = input("Enter verse and versions (e.g., 'I John 2:1,NIV,TEV' or 'I John 2:1 NIV,TEV') (type 'exit' to go back to mode selection): ").strip()
            
            if user_input.lower() in ['exit', 'quit']:
                print("Exiting Verse Comparison mode...")
                break
        ...
        print("Invalid mode. Please select 'conversation' or 'verse', or type 'quit' to exit.")